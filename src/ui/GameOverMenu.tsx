  ...
  return (
    <div style={overlayStyle}>
      <div style={panelStyle}>
        <div style={{ fontSize: 26, fontWeight: 700 }}>Game Over</div>

        {summary && (
          <div style={{ marginTop: 12, fontSize: 14, opacity: 0.95, lineHeight: 1.55 }}>
            <div><b>Time:</b> {summary.timeSurvivedSec.toFixed(1)}s</div>
            <div><b>Score:</b> {summary.finalScore}</div>
            <div><b>Final level:</b> {summary.finalLevel}</div>
            <div><b>Kills:</b> {summary.kills}</div>
            <div><b>Boss kills:</b> {summary.bossKills}</div>
        ...
        <div style={{ opacity: 0.85, marginTop: 8 }}>
          Press R to restart, or use buttons below.
        </div>

        <button style={buttonStyle} onClick={onRestart}>Restart Run</button>
        <button style={buttonStyle} onClick={onMenu}>Back to Menu</button>
      </div>
    </div>
  );
}
